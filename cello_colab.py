...
def main():  # メインの関数
    ...
    for cel_1 in range(1, 8+1):
        for cel_2 in range(1, 8+1):
            ...
            # カラーバーを付加する
            plt.colorbar(im)
            plt.title("1枚目の枚数:"+str(cel_1)+"枚、2枚目の枚数:"+str(cel_2)+"枚の場合")

            # グラフを表示する
            # plt.show()
            plt.savefig("fig/"+str(cel_1)+"-"+str(cel_2)+".png")
